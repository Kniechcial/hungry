	...
	<div class="content">
		<div class="card flex justify-content-center">
			<div class="flex flex-column gap-2">
				<label for="recipe">Tag name</label>
				<InputText
					id="recipe"
					v-model="foodName"
					:feedback="false"
					aria-describedby="recipe-help" />
				<small id="recipe-help">Enter tag to find recipes.</small>
			</div>
		</div>
	</div>
...
import carrotDialog from "@/views/RecipeView/New-recipe/carrotDialog.vue";
const foodName = ref(null);
const value = ref(null);
const recipeStore = useTagStore();
...
async function getRecipe() {
	...
	await recipeStore.getRecipes(0, 5, foodName.value);
